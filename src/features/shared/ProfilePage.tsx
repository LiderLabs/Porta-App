...
export function ProfilePage() {
  ...
  const roleLabel = { admin:"Admin", receptionist:"Receptionist", employee:"Staff", pa:"PA / Secretary", dept_head:"Department Head", superadmin:"Super Admin" };
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPwSaved(true); setCurrentPw(""); setNewPw(""); setConfirmPw(""); setPwStep(false);
      setTimeout(() => setPwSaved(false), 3000);
    } catch (e) { setPwError((e as any)?.errors?.[0]?.message ?? "Failed to update password."); }
  ...
  const css = `
    ...
    .prof-readonly-note { font-size: .75rem; color: var(--muted); margin-top: 10px; font-style: italic; }
    ...
    @media(max-width:600px) { .prof-page { padding: 20px 14px; } }
  `;
    ...
    <>
      ...
      <div className="prof-page">
        ...
        <div className="prof-avatar-row">
          ...
          <div className="prof-avatar-info"><h2>{displayName}</h2><p>{email}</p></div>
        </div>
        <div className="prof-card">
          <div className="prof-card-title">Personal information</div>
          <div className="prof-field"><label className="prof-label">Full name</label><input className="prof-input" value={displayName} disabled /></div>
          <div className="prof-field"><label className="prof-label">Role</label><input className="prof-input" value={roleLabel[role as keyof typeof roleLabel] ?? role} disabled /></div>
          <div className="prof-field"><label className="prof-label">Email address</label><input className="prof-input" value={email} disabled /></div>
          <p className="prof-readonly-note">Your name and role are managed by your administrator.</p>
        </div>
        <div className="prof-card">
          ...
          {!pwStep ? (
            ...
            <>
              <div className="prof-field"><label className="prof-label">Current password</label><input className="prof-input" type="password" value={currentPw} onChange={e => setCurrentPw(e.target.value)} placeholder="••••••••" /></div>
              <div className="prof-field"><label className="prof-label">New password</label><input className="prof-input" type="password" value={newPw} onChange={e => setNewPw(e.target.value)} placeholder="Min. 8 characters" /></div>
              <div className="prof-field"><label className="prof-label">Confirm new password</label><input className="prof-input" type="password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} placeholder="Repeat new password" /></div>
              ...
              <div style={{display:"flex",gap:"8px",marginTop:"14px"}}>
                <button className="prof-save-btn" onClick={handleChangePassword} disabled={pwSaving || !currentPw || !newPw || !confirmPw}>{pwSaving ? "Updating..." : "Update password"}</button>
