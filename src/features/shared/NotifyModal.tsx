...
interface NotifyModalProps {
  ...
  target: "visitor" | "host";
  title?: string;
  /** Optional: fires with the message text when user clicks in-app send */
  onInApp?: (message: string) => void;
}

// ── modal ─────────────────────────────────────────────────────────────────
export function NotifyModal({ isOpen, onClose, template, data, target, title, onInApp }: NotifyModalProps) {
  const [msg, setMsg] = useState(() => buildMessage(template, data));
  const [sent, setSent] = useState<"whatsapp" | "email" | "inapp" | null>(null);
  ...
  const sendEmail = () => {
    if (!email) return;
    const subject = encodeURIComponent(`Porta — ${template === "approved" ? "Visit Approved" : template === "rejected" ? "Visit Update" : "Visit Notification"}`);
  ...
  const sendInApp = () => {
    if (!onInApp) return;
    onInApp(msg.replace(/\*/g, "").replace(/\n/g, " ").trim());
    setSent("inapp");
  };

  const hasWhatsApp = !!phone;
  const hasEmail = !!email;
  const hasInApp = !!onInApp;
  const hasAny = hasWhatsApp || hasEmail || hasInApp;
      ...
      >
        ...
        <div style={{ padding: "14px 20px 0" }}>
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "var(--muted, #8b949e)", marginTop: 2, display: "flex", flexWrap: "wrap", gap: 6 }}>
                {phone && <span>📱 {phone}</span>}
                {email && <span>✉️ {email}</span>}
                {hasInApp && <span>💬 In-app</span>}
                {!hasAny && <span style={{ color: "var(--red, #f85149)" }}>No contact info available</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "14px 20px" }}>
          ...
          <div style={{ fontSize: 11, color: "var(--muted, #8b949e)", marginTop: 6 }}>
            Edit freely. *text* = bold in WhatsApp.
          </div>
        </div>
        ...
        {sent && (
          <div style={{
            ...
            ✓ {sent === "whatsapp" ? "WhatsApp opened — hit Send in WhatsApp to deliver"
              : sent === "email" ? "Email client opened"
              : "Message sent via in-app"}
          </div>
        )}
        ...
        <div style={{ padding: "0 20px 20px", display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {hasInApp && (
            <button onClick={sendInApp} style={{
              flex: 1, minWidth: 120, padding: "11px 0",
              background: "rgba(88,166,255,0.15)", border: "1px solid rgba(88,166,255,0.3)", borderRadius: 10,
              color: "var(--blue, #58a6ff)", fontWeight: 700, fontSize: 13,
              cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
              fontFamily: "inherit", transition: "opacity .15s",
            }}
