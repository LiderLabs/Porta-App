...
function printBadge(visit: any) {
  ...
  if (!w) return;
  w.document.write(`<!DOCTYPE html><html><head><title>Visitor Badge</title>
<style>
  ...
  return (
    <div className="calendar-card">
      <div className="calendar-nav">
        <button className="btn-ghost" onClick={()=>setCur(new Date(year,month-1,1))}>&lsaquo;</button>
        <span className="calendar-month">{MONTHS[month]} {year}</span>
        <button className="btn-ghost" onClick={()=>setCur(new Date(year,month+1,1))}>&rsaquo;</button>
      </div>
      <div className="calendar-live-badge">&bull; LIVE</div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:16,background:"var(--card)",
        ...
        {isOnsite&&(
          <div style={{display:"flex",gap:8,flexShrink:0}}>
            ...
            <button className="action-btn action-btn--badge" onClick={()=>printBadge(v)}>\U0001f5a8 Badge</button>
          </div>
        )}
      </div>
    );
